...
func openSource(cmd *cobra.Command) (mls.Source, error) {
	return postgres.New(cmd.Context(), cfg.Database.URL, postgres.Options{Schema: cfg.Database.Schema})
}
...
func runServe(cmd *cobra.Command, _ []string) error {
	...
	srv, err := server.New(src, server.WithInfo(cfg.Server.Name, version))
	if err != nil {
		return err
	}
	slog.Info("serving mlsgrid-mcp over stdio", "schema", cfg.Database.Schema)
...
and that the sync pipeline is producing current data.`,
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		fmt.Printf("features: geo=%t price_history=%t open_houses=%t sql=%t\n", caps.Geo, caps.PriceHistory, caps.OpenHouses, caps.SQL)
